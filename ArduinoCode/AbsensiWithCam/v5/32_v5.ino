...
uint8_t esp32camMac[] = { 0x78, 0xE3, 0x6D, 0xDD, 0xB6, 0x78 };
...
void onDataReceived(const esp_now_recv_info* info, const uint8_t* data, int len) {
  ...
  } else if (len == sizeof(RtcSyncData)) {
    RtcSyncData rtcData;
    memcpy(&rtcData, data, sizeof(rtcData));
    
    int y, mo, d, h, mi, s;
    if (sscanf(rtcData.timestamp, "%d-%d-%d %d:%d:%d", &y, &mo, &d, &h, &mi, &s) == 6) {
      rtc.adjust(DateTime(y, mo, d, h, mi, s));
      Serial.printf("⏰ RTC disinkronkan ke: %s\n", rtcData.timestamp);
      showOnOLED("RTC Sync", "Berhasil");
      beep(2);
    } else {
      Serial.printf("⚠️ Format waktu salah: %s\n", rtcData.timestamp);
      showOnOLED("RTC Sync", "Format Salah");
      beep(1);
    }
