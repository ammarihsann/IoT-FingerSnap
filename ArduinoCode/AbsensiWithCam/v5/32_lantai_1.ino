...
const char* mqtt_server = "XXXXXXXXXXXXXX.s1.eu.hivemq.cloud";
const int mqtt_port = 8883;
const char* mqtt_username = "hivemq.webclient.XXXXXXXXXXXXXXXXXXXXX";
...
const unsigned long IMAGE_TIMEOUT = 5000; // 5 detik timeout
...
void setup() {
  ...
  client.setBufferSize(12288);  // buffer besar untuk gambar
...
void loop() {
  ...
  // Timeout gambar
  if (receivingImage && millis() - imageStartTime > IMAGE_TIMEOUT) {
    Serial.println("⚠ Timeout: Gambar tidak selesai diterima. Dihapus.");
  ...
  while (Serial2.available()) {
    if (!receivingImage) {
      ...
      while (Serial2.available() && imageBuffer.size() < expectedImageSize) {
